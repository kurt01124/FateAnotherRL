...
namespace Cirnix.JassNative.Runtime
{
    public static class PluginSystem
    {
        ...
        private static readonly string[] pluginFiles = new string[]
        {
            "Cirnix.JassNative.Common.dll",
            "Cirnix.JassNative.dll",
            "Cirnix.JassNative.YDWE.dll",
            "RLCommPlugin.dll",
        };
        ...
        internal static void LoadPlugins(string folder)
        {
            try
            {
                ...
                foreach (var pluginName in pluginFiles)
                {
                    string file = Path.Combine(folder, pluginName);
